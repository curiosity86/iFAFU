...
public class Schedule extends GridLayout {
    ...
    private int dp1 = DensityUtils.dp2px(getContext(), 1F);
    ...
    public void setItemClickListener(final OnItemClickListener listener) {
        if (listener != clickListener) {
            ...
            for (Map.Entry<ScheduleItem, TextView> e : itemViewMap.entrySet()) {
    ...
    public void setDateTexts(String[] dateTexts) {
        if (dateTexts == null || this.dateTexts == dateTexts) {
            return;
        }
        this.dateTexts = dateTexts;
        int loop = Math.min(dateTexts.length, weekLayouts.length);
    ...
    public void addItems(final Collection<ScheduleItem> items) {
        for (ScheduleItem item : items) {
            addItem(item);
        }
    }

    public void addItem(final ScheduleItem item) {
        ...
        itemView.setBackgroundColor(config.colorPool.getColor(item));
    ...
    public void setItems(final Collection<ScheduleItem> items) {
    ...
    private void initTimeTextView() {
        ...
        for (int i = 0; i < loop; i++) {
            TextView timeTextView = noteLayouts[i].findViewById(R.id.id_time_textview);
            if (timeTextView == null) {
