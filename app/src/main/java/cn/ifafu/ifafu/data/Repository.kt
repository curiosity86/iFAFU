...
import cn.ifafu.ifafu.data.entity.exception.VerifyException
import cn.ifafu.ifafu.data.local.AppDatabase
import cn.ifafu.ifafu.data.network.APIManager
import cn.ifafu.ifafu.data.network.parser.*
            ...
            SyllabusParser(user).parse(html).apply {
                ...
                if (!data.isNullOrEmpty()) {
                    ...
                    saveSetting(course.apply {
                        HttpUtils.get("https://api.ifafu.cn/public/text/firstWeek").body()?.string()?.run {
                            val jo = JSONObject.parseObject(this)
                            if (jo.containsKey("content")) {
                                val content = jo["content"].toString().substring(0, 10)
                                kotlin.runCatching {
...
//                    Log.d("Holiday Calc", "holiday week = $holidayWeek    $i")
        ...
        suspend fun holidayChange(oldCourses: List<Course>): MutableList<MutableList<CourseBase>?> = withContext(Dispatchers.Default) {
            //MutableMap<fromWeek, MutableMap<fromWeekday, Pair<toWeek, toWeekday>>>
            val fromTo = Repository.syllabus.getAdjustmentInfo()
            //按周排列课程
